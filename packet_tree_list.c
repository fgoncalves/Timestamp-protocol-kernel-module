...
static pkt_list_node* make_pkt_node(__be32 srcaddr){
  pkt_list_node* pn = (pkt_list_node*) kmalloc(sizeof(pkt_list_node), GFP_ATOMIC);
...
pkt_list* make_pkt_list(void){
  pkt_list* pn = (pkt_list*) kmalloc(sizeof(pkt_list), GFP_ATOMIC);
...
packet_tree* get_packet_tree(pkt_list* l, __be32 srcaddr){
  ...
  for(npkt = l->first; npkt != NULL; npkt = npkt->next)
    if(npkt->srcaddr == srcaddr)
      return npkt->pkt_t;
  return NULL;
}
